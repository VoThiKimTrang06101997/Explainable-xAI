...
from verl.utils.reward_score import exact
...
def main_task(config, compute_score=None):
    ...
    elif reward_manager_name == 'exact_rank':
        from exact_modules.reward.rank_bonus_manager import ExactRankBonusRewardManager
        reward_manager_cls = ExactRankBonusRewardManager
    else:
        raise NotImplementedError
    ...
    if config.actor_rollout_ref.model.path.strip().startswith("Qwen") or 'llama' in config.actor_rollout_ref.model.path.lower() or config.actor_rollout_ref.model.use_think == False:
        print("\nQwen or LLAMA---------------------------------\n")
        if "exact" in str(config.data.train_files).lower():
            compute_score = exact.compute_score
        else:
